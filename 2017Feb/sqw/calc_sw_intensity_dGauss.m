function [D,x0,alpha,e_sw,Icr,dIcr,all_plots]=calc_sw_intencity_dGauss(data_source,bragg,cut_direction,cut_p,dE,dK)
...
for i=1:size(cut_p,1)
    ...
    IP=w0.data.s;
    dIP = w0.data.e;
    [fw1,par]=fit(w1,@gauss2Ibkgd,[IP,q_sw(i),peak_width,0.01,0],free_params);
...
qq_min = min(fp(max(e_sw),parR),fm(max(e_sw),parR));
qq_max = max(fp(min(e_sw),parR),fm(min(e_sw),parR));
xxpf=qq_min:0.01:qq_max;
...
%
function y = gauss2Ibkgd(x, p, varargin)

y=exp(-0.5*((x-p(2))/p(3)).^2)*(p(1)/sqrt(2*pi)/p(3))+ ...    
  exp(-0.5*((x+p(2))/p(3)).^2)*(p(1)/sqrt(2*pi)/p(3))+ ...
    + (p(4)+x*p(5));
...
mina = min(q_min0,q_max0);
maxa = max(q_min0,q_max0);
