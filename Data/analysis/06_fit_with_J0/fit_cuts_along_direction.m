function fit_res = fit_cuts_along_direction(...
    the_2Dcut,cut_name_base,direction_name,cut_en,dE_step,half_dE_range,use_mask,mask_par,cut_range_curvature)
...
res_name = sprintf("%s_2D%s_dir%s_dE%d.mat",cut_name_base,pref,direction_name,2*half_dE_range);
res_name = fullfile(data_path,res_name);
if isfile(res_name)
    ld = load(res_name);
    ...
    for i = 1:N_points
        en = cut_en(i);
        if use_mask
            w2 = cut(the_2Dcut,[0,0.02,1],[en-half_dE_range ,dE_step,en+half_dE_range]);
            w1 = cut(w2,[],[en-half_dE_range ,en+half_dE_range]);
        else
            max_q = interp1(e_lim,q_par,en);
            w2 = cut(the_2Dcut,[0,0.02,max_q],[en-half_dE_range,dE_step,en+half_dE_range]);
            w1 = cut(w2,[],[en-half_dE_range ,en+half_dE_range]);
        end
        acolor k;
        plot(w1);
        hkl_proj = w1.data.proj;
        hkl_proj.offset = 0;
        ...
        % acolor r;
        % w1fit = cut(fit_obj,[],[en-half_dE_range ,en+half_dE_range]);
        % pl(w1fit);
        % 
    ...
    en_bins = [cut_en-half_dE_range,cut_en(end)+half_dE_range];
