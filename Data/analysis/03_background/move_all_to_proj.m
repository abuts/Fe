...
% cut projection's coordinate system.
%
% Inputs:
...
%                PageOp_sqw_binning algorithm tansformed into coordinate
...
%
% done for 2-D cuts for performance to avoid interal loop
%--------------------------------------------------------
...
% get cut ranges of the image to combine everything in
...
for i=1:numel(proj_array)
    ...
    % image assiciated with proj_array(i) projection
    coord_tr = proj_array(i).transform_pix_to_img(q_coord);
    % find the data faling outside of the projection of interest range
    ...
    % projection into the Crystal Cartesian coordinats system related with
    % target projection.
