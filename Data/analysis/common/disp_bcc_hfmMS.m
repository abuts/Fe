function w = disp_bcc_hfmMS(qh,qk,ql,par)
...
if JS_p5p5p5~=0
    w(:,1) = w(:,1) + (8*JS_p5p5p5)*(1-cos1h.*cos1k.*cos1l);
end

if JS_100~=0
    w(:,2) = w(:,2) + (4*JS_100)*(sin(pi*qh).^2 + sin(pi*qk).^2 + sin(pi*ql).^2);
end

if JS_110~=0
    w(:,3) = w(:,3) + (4*JS_110)*(3 - cos2h.*cos2k - cos2k.*cos2l - cos2l.*cos2h);
end
...
%     cos3k = cos((3*pi)*qk);
%     cos3l = cos((3*pi)*ql);
%     w(:,4) = w(:,4) + (8*JS_3p5p5p5)*(3 - cos3h.*cos1k.*cos1l - cos3k.*cos1l.*cos1h...
%         - cos3l.*cos1h.*cos1k);
% end
% 
% if JS_111~=0
%     w(:,5) = w(:,5) + (8*JS_111)*(1-cos2h.*cos2k.*cos2l);
% end
